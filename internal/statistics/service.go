...
func (s Service) authorize(ctx context.Context, membership domain.Membership, permission domain.PermissionKey) (dashboardContext, error) {
	...
	err := queryer.QueryRowContext(ctx, `SELECT settings.statistics_enabled,settings.settlements_enabled,g.currency,notifications.timezone,g.created_at
		FROM groups g
		JOIN group_settings settings ON settings.group_id=g.id
		JOIN group_notification_settings notifications ON notifications.group_id=g.id
