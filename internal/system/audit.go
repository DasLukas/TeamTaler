...
// ListAudit returns at most limit newest-first immutable instance events. An
// invalid limit uses the safe default of 100. Malformed legacy metadata is
// represented by {"decodeError": true}; query and scan failures are returned.
// Example: events, err := service.ListAudit(ctx, 50).
func (s Service) ListAudit(ctx context.Context, limit int) ([]AuditEvent, error) {
	if limit < 1 || limit > 200 {
		limit = 100
	}
	rows, err := s.db.QueryContext(ctx, `SELECT id,actor_user_id,action,resource_type,
		resource_id,metadata_json,occurred_at
		FROM system_audit_events ORDER BY occurred_at DESC,id DESC LIMIT ?`, limit)
	if err != nil {
	...
	for rows.Next() {
		...
		var metadata string
		if err := rows.Scan(&event.ID, &event.ActorUserID, &event.Action, &event.ResourceType,
