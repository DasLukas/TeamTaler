...
- A dedicated permission-protected finance workspace at `/finance` with lifecycle-grouped member balances, exact receivable/credit/net totals, payment management, and period settlements. Archived accounts remain payable; permanently removed accounts appear only while a later correction leaves a non-zero balance.
...
- Typed group-administration behavior settings with a safe default role, optional notification emails, transaction reason rules, editable payment methods, and separate booking and payment reason suggestions.
