...
The media and receipt limits are editable without a restart through the System tab or `teamtaler admin system settings set`. Media must be a whole MiB value from 1 through 25 MiB; receipts must be a whole MiB value from 1 through 50 MiB. The server applies each live value plus a 1-MiB multipart reserve to the matching routes, independently of `TEAMTALER_MAX_REQUEST_BYTES`. Product images, group logos, and profile images retain one metadata-free content-addressed PNG master and receive bounded 128-, 256-, and 384-pixel WebP display variants. Existing masters create missing variants on first display, so upgrades and restores require no media migration. The responsive client requests only the appropriate display size, keeps a stable fallback until decoding completes, and lazily loads media outside the initial viewport. Authorized managed-image responses use private conditional caching with strong ETags; every reuse revalidates authorization before a stored body may be displayed. Receipt images are decoded, dimension-checked, metadata-stripped, and normalized; bounded PDFs are validated and stored as opaque documents and remain uncached. Configure a reverse proxy for at least 51 MiB so it does not override the maximum receipt setting.

Group administrators configure receipt handling independently for every payment method as disabled, optional, or required. New groups start with Bank transfer without a receipt, Shopping with a required receipt, Cash and PayPal without receipts, and Other with an optional receipt. Members may select a JPEG, PNG, WebP, or PDF from the device, use a photo-library source, or create a locally processed multi-page PDF through the camera-only document scanner. Imported files remain separate from camera scan sessions. A payment keeps exactly one immutable receipt, including after reversal. The affected member and current holders of `FINANCE_MANAGEMENT` may retrieve it; storage identifiers are never exposed through the API. Retrieved images open in the in-app receipt dialog with a shared zoom, rotation, reset, and download toolbar; PDFs open in a new native browser preview tab through the same protected blob flow used by table PDF exports.

Groups may optionally model external cash, bank, PayPal, and provider-neutral accounts inside the Finance workspace. `VIEW_EXTERNAL_ACCOUNTS` grants balances, links, history, and exports; `MANAGE_EXTERNAL_ACCOUNTS` additionally grants account configuration, manual income and expense entries, transfers, and reversals. `FINANCE_MANAGEMENT` implies both rights. Only `GROUP_ADMINISTRATION` may switch the feature on or off, and that switch alone does not disclose account data. Disabling the feature hides manual access without deleting history; payments already connected to an account and their reversals continue to update its ledger-derived balance.

Each payment method may reference one external account, while several methods may reference the same account. Unlinked methods retain the legacy group-cash posting. The group administration explains the distinction between a payment method and its optional external-account assignment; operational payment dialogs show only the selected method and do not expose accounting effects. Bank and PayPal account details are the single persisted source for the existing member payment instructions: a bank account carries a normalized SEPA recipient name, validated IBAN, and optional BIC, while a PayPal account carries a normalized PayPal.Me handle. Distinct bank targets may share an IBAN when their recipient or BIC differs; identical targets reuse one account. While external accounts are disabled, changed legacy payment targets are saved without an explicit account-link field, leaving unrelated links untouched. Save any outstanding payment-method edits before switching the feature; the switch is unavailable while those edits are unsaved. Group finance and booking cards save independently, while reason rules and feature switches save immediately. System settings save within each section, and changing roles with unsaved edits asks for confirmation. SEPA accounts are available only to EUR groups. These values are operational member data rather than credentials: active members still receive the linked destination through transaction settings, and authorized group raw-data exports include the complete account configuration. TeamTaler stores the values as clear text in SQLite and backups. Obtain the recipient's consent and apply the same access and retention policy as for other personal financial data.

Manual external-account income, expenses, and transfers may carry one optional immutable JPEG, PNG, WebP, or PDF evidence attachment. The transaction dialog reuses the payment receipt picker, including local multi-page scanning, validation, preview, and the live instance attachment limit. Reversals never rewrite or duplicate the original attachment; a newly posted independent transaction may carry its own evidence. Members with live `VIEW_EXTERNAL_ACCOUNTS` access may retrieve these group-owned attachments from the account history.

External-account balances are never overwritten or imported from a provider. They are calculated from immutable double-entry ledger rows created by opening balances, linked TeamTaler payments, manually recorded external income or expenses, and transfers. Negative balances are valid and shown as warnings. A reversal appends exact counter-entries; a different amount or account is recorded as a separate new income, expense, or transfer. Archiving an account atomically detaches its current payment methods while retaining all historical ledger references. An archived, unlinked, unused account is physically deleted. An archived, unlinked account with an exact zero balance may also be removed from everyday management, but its hidden identity remains reserved so immutable history, searches, and exports retain the original account name and type. A later reversal that produces a non-zero balance automatically returns that identity to the archived account list and records the restoration in the audit log. Active accounts and accounts with non-zero balances cannot be deleted.
...
The scanner initializes a portable detector through an explicit worker-readiness handshake before scheduling bounded RGBA camera frames. Its CSP-safe typed-array pipeline performs adaptive edge extraction, gradient-guided line detection, confidence-gated quadrilateral scoring, candidate smoothing, and automatic capture without runtime code generation, WebAssembly, `ImageBitmap`, worker-side canvas, or DOM access, including on mobile Safari. The overlay stays aligned with the actual contained camera image. Initialization and repeated frame-transfer failures stop with a visible manual-capture fallback instead of leaving automatic detection in an indefinite preparing state. Manual capture uses default editable corners unless a recent validated contour exists. The editor provides Original, optimized Color, and Grayscale modes. Color performs bounded white balancing, tonal normalization, restrained saturation, and sharpening; Grayscale performs luminance-based contrast normalization and sharpening. The editor enhancement preview, committed page thumbnail, and generated PDF share the deterministic pixel implementation. Committed thumbnails show the perspective-corrected crop, rotation, and selected filter, while inverse-homography rasterization samples every output pixel once and cannot expose triangle-mesh seams in a reopened PDF.
...
Delivered messages use German multipart email with a plain-text fallback, a responsive HTML card, client-adaptive light and dark colors, and a CID-embedded PNG logo; they load no remote email assets. Group messages show the current group name and logo and use the recipient membership theme override followed by the group default. Invitations and public-join verification for recipients without an active membership use the group default. Missing group images fall back to the TeamTaler mark without blocking delivery. Password-reset, email-change, and SMTP-test messages always use TeamTaler branding. Group subjects begin with the group name; system subjects begin with `TeamTaler`.
...
Raw exports are ZIP archives containing UTF-8 CSV datasets, `manifest.json`, and `schema.json`. They use stable English technical headers, RFC 3339 UTC timestamps, and exact minor-unit integer strings. They contain structured database fields and attachment metadata only. The complete group archive includes external accounts, payment-method links, immutable external-account transactions and attachment metadata, related ledger references, and clear-text PayPal.Me or SEPA recipient fields; the personal archive adds none of this group-owned configuration or history. Group logos, profile and product images, payment and external-transaction attachment bytes, secrets, password hashes, sessions, and other media are never copied into either archive. Archives are derived sensitive data. Keep the application data volume private, download them only over HTTPS, remove local copies when no longer needed, and do not treat the 24-hour application retention as a policy for browser downloads or backups.
...
The External account transactions table follows the same CSV/PDF contract and additionally requires the live external-account feature plus `VIEW_EXTERNAL_ACCOUNTS` both when rendering begins and immediately before the artifact is delivered.
...
Migration `0054` moves configured PayPal.Me and SEPA destinations into external-account records, links only methods with identical destination details to the same account, and leaves historical `GROUP_CASH` ledger rows unchanged. In particular, equal IBANs with different recipient names or BICs remain distinct payment instructions. The external-account feature remains disabled for every group after upgrade. Because the migration rebuilds payment methods and the immutable ledger to add tenant-bound references, create and verify a restorable backup, provide free space for a database copy plus WAL, deploy API and SPA together, and use a maintenance window. Do not downgrade the migrated database; restore the pre-upgrade backup instead.

Migration `0055` additively introduces immutable one-to-one evidence metadata for manual external-account transactions. It is safe for databases that already applied `0054`; attachment bytes continue to use the shared content-addressed receipt store and backup inventory.

Migration `0056` replaces the early IBAN-only bank-account uniqueness index on databases that already applied an older `0054` draft. Bank targets with the same IBAN but different recipient or BIC details can then coexist. Previously overwritten recipient details cannot be reconstructed from that older schema; restore a pre-migration backup if those original values are required.
...
Application request logs include the final response status but not request or response bodies. Payment-method and external-account audit metadata records only stable identifiers, transaction/account types, exact amounts, and aggregate counts. Account names, reasons, references, notes, recipient names, PayPal.Me handles, IBANs, and BICs remain present where required in persisted finance records, database backups, and complete group raw-data exports but are excluded from application-generated request logs and audit metadata.
