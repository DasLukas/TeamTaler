...
- `internal/finance` implements consolidated member accounts, lifecycle-aware active/archived/deleted balance summaries, personal and anonymous group category statistics, permission-gated group and own-account incoming payments, configured payment-method validation and label snapshots, payment reversal, recent ledger activity, and finance-management read models. Archived accounts remain settlement targets; deleted accounts are projected only while their current receivable is non-zero.
...
- `web/src/features` contains authentication, dashboard, booking, activity, account, notifications, permission-aware catalog, permission-aware finance, and administration slices. Administration combines group identity, branding, notification delivery, default-role policy, and transaction behavior in the Settings workspace; it separates membership, invitation, and guest lifecycle plus subject-centered role assignments in the members workspace from role definitions and grants in the roles workspace. The booking slice consumes a dedicated minimal context, groups guests after regular targets, and submits new names only as part of one batch command. The notification slice owns the shell-level unread summary, shared badges, cursor-backed inbox, viewport acknowledgement, and retry state. The catalog slice owns the guarded `/catalog` workspace, contextual category/product creation, the accessible icon picker, and nested pointer/touch/keyboard catalog sorting. The finance slice owns the `/finance` overview, payment, and settlement tabs plus the reusable reviewed self-payment dialog shared by the dashboard and account page; the dashboard owns personal information and permission-gated anonymous group aggregates. The shared category-icon renderer maps persisted semantic icon names to tree-shaken Lucide components.
...
- `GET /api/v1/groups/{groupId}/booking-context` returns the open period, own balance, current membership, authorized targets, guest-booking capability, the foreign-booking reason rule, and ordered booking-reason suggestions. Target items expose only membership ID, display name, optional avatar, and credential-derived temporary-guest flag. `GET /members` separately requires `VIEW_MEMBER_DIRECTORY`.
- `GET /api/v1/groups/{groupId}/transaction-settings` exposes only the non-sensitive reason rules, ordered payment methods, and booking/payment suggestions required by active-member forms. Administrator writes remain part of the `GROUP_ADMINISTRATION`-protected group-settings transaction.
