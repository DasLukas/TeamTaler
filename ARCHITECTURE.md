...
- `web/src/features` contains authentication, dashboard, booking, activity, account, notifications, permission-aware catalog, permission-aware finance, and administration slices. Administration combines group identity, branding, notification delivery, default-role policy, transaction behavior, and a dedicated Finance section with the settlement switch in the Settings workspace; it separates membership, invitation, and guest lifecycle plus subject-centered role assignments in the members workspace from role definitions and grants in the roles workspace. The booking slice consumes a dedicated minimal context, groups guests after regular targets, and submits new names only as part of one batch command. The notification slice owns the shell-level unread summary, shared badges, cursor-backed inbox, viewport acknowledgement, and retry state. The catalog slice owns the guarded `/catalog` workspace, contextual category/product creation, the accessible icon picker, and nested pointer/touch/keyboard catalog sorting. The finance slice owns the `/finance` overview, payment, and optional settlement/history tabs plus the reusable reviewed self-payment dialog shared by the dashboard and account page; the dashboard owns personal information and permission-gated anonymous group aggregates. Current-period presentation is omitted while settlements are disabled, whereas immutable history remains conditionally available when it exists. The shared category-icon renderer maps persisted semantic icon names to tree-shaken Lucide components.
...
| `group_settings` | One typed group-administration behavior record per group, including required `settlements_enabled` defaulting to false, optional notification email delivery, a nullable tenant-bound default-role reference, and the inert activity-visibility compatibility field. |
...
- `GroupSettings` and the active-member `TransactionSettings` projection expose required `settlementsEnabled`; `GroupSettingsUpdate` accepts optional `settlementsEnabled` for partial administration updates. Missing values from a pre-upgrade server are adapted to false by the SPA during a coordinated rollout.
