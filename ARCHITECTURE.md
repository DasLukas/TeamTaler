...
TeamTaler is a self-hosted group expense and settlement application implemented as a modular monolith. One Go process serves the versioned JSON API and the compiled React single-page application. The same process owns authentication, authorization, managed and login-enabled guest lifecycle, domain transactions, SQLite access, image delivery, optional SMTP invitation, public-registration-verification, and notification delivery, health endpoints, and request logging.
...
- `internal/auth` implements Argon2id password hashing, first-run bootstrap, login, rate-limited secret-minimal invitation and public-link previews, public email-verified registration, ordinary invitation and managed-guest claim acceptance, archived-membership reactivation with role replacement, profile-image lifecycle, opaque server-side sessions, logout, and expired-session cleanup. Credentialless guest identities are deliberately excluded from authentication queries.
- `internal/groups` implements group creation, tenant membership lookup, permission-gated identity and behavior settings, atomic guest-role setup, privacy-gated member listing, managed-guest rename/claim/archive lifecycle, group-owned role and grant management, optimistic role assignments, versioned public join-link lifecycle, individual invitation creation/editing/revocation/resending, shared invitation deduplication, and atomic idempotent CSV invitation imports.
...
- `internal/bookings` resolves server-authoritative fixed prices or validates actor-supplied unit prices, then implements idempotent single and atomic multi-target booking creation, transaction-bound managed-guest creation, privacy-minimized booking context, immutable product/category/price snapshots, permission-gated third-party assignment, actor-based 30-second reason-free reversal, audited reversal, and activity visibility.
...
- `web/src/features` contains authentication, dashboard, booking, activity, account, notifications, permission-aware catalog, permission-aware finance, and administration slices. Administration separates group, membership, invitation, and guest lifecycle plus subject-centered role assignments in the members workspace from role definitions and grants in the roles workspace. The booking slice consumes a dedicated minimal context, groups guests after regular targets, and submits new names only as part of one batch command. The notification slice owns the shell-level unread summary, shared badges, cursor-backed inbox, viewport acknowledgement, and retry state. The catalog slice owns the guarded `/catalog` workspace, contextual category/product creation, the accessible icon picker, and nested pointer/touch/keyboard catalog sorting. The finance slice owns the `/finance` overview, payment, and settlement tabs plus the reusable reviewed self-payment dialog shared by the dashboard and account page; the dashboard owns personal information and permission-gated anonymous group aggregates. The shared category-icon renderer maps persisted semantic icon names to tree-shaken Lucide components.
...
The authenticated route `/book` requires `CREATE_OWN_BOOKING` or `BOOK_FOR_OTHERS` and reads `/booking-context` rather than dashboard or member-directory queries. `/overview` contains personal member information, recent activity, a `VIEW_GROUP_STATISTICS`-gated anonymous category aggregate, a separately `FINANCE_MANAGEMENT`-gated total, and permission-specific actions such as the `RECORD_OWN_PAYMENT`-gated balance action. The same payment action is available on `/account`. It opens a mobile bottom sheet or desktop dialog with entry, review, and success states. `/activities` uses server-resolved booking actor/target names, so it does not fetch the protected directory. `/catalog` requires `CATALOG_MANAGEMENT`, while `/finance` requires `FINANCE_MANAGEMENT` and derives payment targets from finance-authorized account summaries. `/admin` exposes group and guest lifecycle, public join-link management, and reserved-administrator transfer with `GROUP_ADMINISTRATION`; its Members tab requires `VIEW_MEMBER_DIRECTORY` together with either `GROUP_ADMINISTRATION` or `ROLE_MANAGEMENT`, and its Roles & Rights tab requires `ROLE_MANAGEMENT`. Lifecycle controls, ordinary assignments, and protected assignments remain independently permission-gated, and unauthorized panels do not mount their queries. Public routes `/join` and `/join/verify` consume secrets exclusively from URL fragments; `/join` offers authentication for existing accounts and verified registration for new accounts. `/` and post-authentication transitions choose the first permitted destination in the order booking, finance, catalog, administration, overview. The retired `/reports` route redirects to `/overview`; other explicit deep links remain stable and are not replaced by launch routing.
...
| `users` | Global local identities, nullable email/password credentials constrained to be both present or both absent, and optional profile-image keys. Credentialless rows back managed guests but still have stable IDs. |
...
| `group_settings` | One typed group-administration behavior record per group, including optional notification email delivery, a nullable tenant-bound default-role reference, disabled-by-default managed-guest creation, a nullable tenant-bound login-claim-role reference, and the inert activity-visibility compatibility field. |
| `memberships` | User participation, active/archive status, role-assignment collection version, and an optional normalized managed-guest name key that is case-insensitively unique among active guests in a group. |
...
| `invitations` | Hashed single-use tokens, invited email, optional display name, optional managed-guest target membership, role-assignment collection version, expiry, and acceptance/revocation state. |
...
Tenant-bearing queries are scoped by `group_id`. Composite foreign keys protect important group-owned relationships such as default and guest roles, grants, assignments, claim targets, products, bookings, allocations, and ledger references. Scope shape constraints require a group grant to carry no resource identifier, a category grant to reference a category in the same group, and a product grant to reference a product in the same group. Category and product shapes are stored for forward compatibility but are rejected by v1 service and HTTP validation.

Every group has four seeded roles. `GROUP_ADMINISTRATOR` is protected, has the fixed name `Group administrator`, starts with all twelve permissions, and cannot lose `GROUP_ADMINISTRATION` or `ROLE_MANAGEMENT`. `MEMBER` is an editable starter role that begins with `CREATE_OWN_BOOKING`, `VOID_OWN_BOOKING`, `VIEW_MEMBER_DIRECTORY`, and `VIEW_GROUP_STATISTICS`. `FINANCE_MANAGER` starts with `FINANCE_MANAGEMENT`, `VIEW_ALL_BOOKING_ACTIVITY`, `RECORD_OWN_PAYMENT`, and both new read permissions. `CATALOG_MANAGER` starts with `CATALOG_MANAGEMENT` and both new read permissions. All roles except `GROUP_ADMINISTRATOR` may be renamed, modified, or deleted when they have no active-member or pending-invitation assignments. The configured guest role is additionally protected by the group-settings foreign key. Expired invitation assignments do not block deletion and are removed by the role foreign-key cascade. Role names are trimmed, reject control characters, and are case-insensitively unique within a group.

Group-insert triggers seed all four presets. The first active membership receives only the reserved administrator role, which already starts with every capability. Membership removal switches the row to `ARCHIVED`, clears role assignments, and retains the stable membership ID and all dependent history. Ordinary invitation acceptance and reactivation apply exactly the invitation's explicit non-empty role set. Managed-guest claim acceptance instead preserves its target membership and installs only the group's current retained guest role. Every login-enabled active membership and pending invitation must retain at least one assignment; an active membership backed by a credentialless managed identity is the deliberate roleless exception. The reserved administrator role must additionally remain assigned to at least one active membership. A custom role containing `GROUP_ADMINISTRATION` does not satisfy this invariant. Database constraints and serialized service transactions jointly protect the reserved role, default and claim-role references, credential coupling, conditional minimum assignment cardinality, and the last active administrator assignment.

Guest status is not a second membership kind. `Membership.isGuest` is derived from the credentialless identity state or the configured guest-role assignment. Managed guests keep a required `user_id`; their user row has null email and password hash and therefore cannot satisfy login or session queries. Claiming with an existing account may rebind `user_id`, while the membership ID and all accounting references remain stable. Claimed guests have credentials but remain guests through the configured role. No synthetic address, placeholder password, or role-name comparison participates in identity or authorization.
...
| --- | --- |
| `GROUP_ADMINISTRATION` | Manage group identity, behavior and guest settings, membership, guest and invitation lifecycle, audit access, and protected administrator assignment. |
...
| `VIEW_GROUP_STATISTICS` | Read anonymous group-wide category statistics; does not grant finance totals or another member's balance. |
...
| `BOOK_FOR_OTHERS` | Create a reasoned booking for another active membership or a new managed guest; implies `VIEW_MEMBER_DIRECTORY`. |

Implications are computed by policy and are not stored as duplicate grants. The current implication graph is `VOID_ANY_BOOKING -> {VOID_OWN_BOOKING, VIEW_ALL_BOOKING_ACTIVITY}` and `BOOK_FOR_OTHERS -> VIEW_MEMBER_DIRECTORY`. `PermissionGrant` includes a generic resource scope so future policy can decide category or product access through the same interface. v1 rejects every scope except `{type: "GROUP"}` to avoid advertising unenforced restrictions.

An active login-enabled membership retains limited non-administrative base behavior independently of grants: it may read its own account and notifications plus actor/target activity visible to it. Anonymous group category statistics require `VIEW_GROUP_STATISTICS`; member email, role, and grant data requires `VIEW_MEMBER_DIRECTORY`; the group outstanding total remains gated by `FINANCE_MANAGEMENT`. Self-booking requires `CREATE_OWN_BOOKING`; third-party or managed-guest booking requires `BOOK_FOR_OTHERS`, and neither permission implies the other. `VIEW_ALL_BOOKING_ACTIVITY` does not expose another member's ledger or personal dashboard. `RECORD_OWN_PAYMENT` derives its target from the authenticated membership and does not imply finance reads or payment reversal.
...
### Managed guest setup, booking, and claim

1. A group administrator sends `PUT /groups/{groupId}/guest-settings`. Enabling alone permits roleless managed-guest creation. The same command may select an existing role with exactly one group-wide `CREATE_OWN_BOOKING` grant, create that fixed minimal role atomically, or reuse the retained role for later login claims. Existing assignments of a newly selected role must already be exclusive. The transaction rechecks `GROUP_ADMINISTRATION`, tenant ownership, role safety, assignment conflicts, and default-role constraints; when a claim role is selected it becomes both `guest_role_id` and `default_role_id`. Safe template creation does not require `ROLE_MANAGEMENT`.
2. A configured role change returns `409` while the old role has any membership or open-invitation assignment; no mass reassignment is implicit. Disabling only sets `guests_enabled` false. If the guest role is still the global default, the command must replace `default_role_id` atomically. The guest role, classification, identities, claims, and history remain intact.
3. `/booking-context` requires `CREATE_OWN_BOOKING` or `BOOK_FOR_OTHERS` and returns the open period, authenticated membership's exact balance, current membership, minimal allowed targets, and a server-derived guest-creation capability. It never returns target emails, roles, grants, balances, or group statistics. Targets with `isGuest` sort after regular memberships.
4. An idempotent batch booking may contain existing target IDs, managed guest display names, or both, with a combined maximum of 100. The existing-target array may be empty when at least one new name is present. After normalizing names and validating all permissions, targets, product version, period, price, and feature state inside one write transaction, each new name receives a credentialless user row, roleless active membership, booking, paired ledger entries, notification, and audit event. No `guest_role_id` is required for this flow. The one stored idempotency result covers the complete response. Any failure rolls every row back. Active managed-guest names are case-insensitively unique; a conflict returns `existingMembershipId` but never selects or merges it implicitly.
5. `GROUP_ADMINISTRATION` may rename an active credentialless guest, archive any guest membership, or create one conflict-protected claim invitation. Rename is a naturally idempotent replacement. Archive retains the stable membership and all dependent records. Claim creation requires a configured retained guest role, allows only one open claim per guest, and accepts no idempotency key, so a repeat returns `409` without token rotation or duplicate delivery.
6. Claim acceptance consumes the invitation once, preserves its target membership ID and all accounting references, attaches credentials or an authenticated existing global user, and replaces role assignments with the current retained guest role only. A conflicting group membership is rejected rather than merging financial histories. Feature disable does not invalidate an already-created claim.
7. Credentialless guests can receive durable in-app events bound to their membership, but notification email outbox creation is skipped while their email is null. After claim, future eligible notifications use the real address; no synthetic address is ever persisted or delivered.
...
sequenceDiagram
    ...
    UI->>HTTP: POST single or batch booking + existing IDs/new guest names + Idempotency-Key
    HTTP->>Booking: Principal, membership, command
    Booking->>DB: Load active product, category, open period
    Booking->>DB: Validate pricing, version, period, permissions, targets, guest settings
    Booking->>DB: Insert roleless guests, booking snapshots, and ledger pairs
...
For `FIXED` products, the server rejects a submitted unit price and calculates the total from the current persisted price. For `USER_DEFINED` products, it requires and bounds the actor-supplied unit price, includes that price in the idempotency hash and audit metadata, and calculates the total as unit price times quantity. The chosen unit price is stored only in the immutable booking snapshot. A stale product version or stale expected period produces a precondition failure. Categories are the only product classification layer; there is no additional standard/penalty type. The batch endpoint accepts existing IDs and normalized managed-guest names with a combined limit of 100, including an empty existing-ID array when names are present, creates one ordinary booking per resolved target in existing-ID-then-new-name request order, and stores the complete result under one idempotency intent. Third-party and guest targets require `BOOK_FOR_OTHERS` and one shared reason, and each receives an in-app notification. Roleless guest identity creation, target validation, booking and ledger inserts, allocation rebuilds, notifications, audits, and the idempotency result share one SQLite transaction, so a failure cannot leave a guest without its booking or a partial batch. A reversal initiated by somebody other than the target produces a second contextual notification.

Every booking snapshot stores both `actor_membership_id` and `target_membership_id`. Booking read models resolve `actorDisplayName` and `targetDisplayName` server-side, allowing activity and dashboard clients to display and search both identities without calling the privacy-sensitive member directory.
...
A member with `FINANCE_MANAGEMENT` supplies the final label, due date, and optional successor label. One transaction closes the only open period, rebuilds every member's allocations, inserts an immutable `period_statements` row and notification for every credentialed membership and every credentialless guest with financial activity in that period, opens a successor period, writes an audit event, and stores the idempotent result. Idle credentialless guests are skipped so one-off identities do not accumulate empty statements or notifications.
...
- Product creation, CSV invitation import, invitation resend, booking creation/reversal, administrative and self-service payment creation, payment reversal, and period close require an `Idempotency-Key`. Managed-guest creation has no separate endpoint or retry scope: it is part of the batch-booking request hash and stored response. Guest settings and rename are deterministic replacements; a second open claim invitation is conflict-protected with `409` instead of accepting an idempotency key.
...
- `PUT /api/v1/groups/{groupId}/guest-settings` atomically configures, creates, reuses, or disables the tenant-bound guest role and returns complete `GroupSettings`. `PATCH` and `DELETE` on a membership rename or archive with `GROUP_ADMINISTRATION`; `POST /members/{membershipId}/claim-invitation` creates the single open claim command.
- `GET /api/v1/groups/{groupId}/booking-context` returns `{openPeriod, ownBalanceMinor, currentMembership, targets, canCreateManagedGuests}`. Target items expose only membership ID, display name, optional avatar, and derived guest flag. `GET /members` separately requires `VIEW_MEMBER_DIRECTORY`.
- Batch booking accepts `targetMembershipIds`, `managedGuestDisplayNames`, or both with at least one and a combined maximum of 100. Booking representations include server-resolved actor/target names, `canVoid`, `voidReasonRequired`, and optional `voidWithoutReasonUntil`; clients must consume these fields rather than infer policy or load the member directory.
...
- The member directory requires `VIEW_MEMBER_DIRECTORY`; `BOOK_FOR_OTHERS` implies it. Anonymous category totals require `VIEW_GROUP_STATISTICS`, while aggregate outstanding money remains finance-gated. The booking context is an explicit data-minimization boundary and never returns target email, role, grant, balance, or group-statistic fields.
- Guest settings, rename, claim, and archive require `GROUP_ADMINISTRATION`. Role setup accepts only an existing exactly minimal role or creates the fixed minimal template. Later role edits use the normal role policy and intentionally change claimed-guest access immediately. A guest-role switch is rejected while the old role remains assigned, preventing silent privilege or classification migration.
...
Bootstrap, group creation, group-name and group-logo updates, atomic guest settings, managed-guest rename/claim/archive, individual and CSV invitation creation, ordinary and claim invitation acceptance, public join-link lifecycle and acceptance, role/grant and assignment replacement, catalog writes, booking commands including inline guest creation, administrative and self-service payment commands, and period close define explicit transaction boundaries. Critical permission mutations use serialized SQLite write transactions that reload actor access, validate the supplied ETag version where applicable, protect reserved/default/guest roles and assignments, persist the change, and write audit metadata before commit. SMTP and image decoding occur outside database transactions after the corresponding durable authorization and work records exist.
...
Migration `0021` rebuilds credential-bearing identity and statement storage so user email/password values are a coupled nullable pair and close-time email may be null, while preserving required membership-to-user references, real-email uniqueness, sessions, profiles, memberships, bookings, ledger entries, and statements. It narrows the minimum-role trigger only for active memberships backed by credentialless users, extends group settings with disabled-by-default guest creation and a nullable composite foreign key to the tenant's login-claim role, and extends invitations with an optional tenant-bound claim target. It adds `VIEW_MEMBER_DIRECTORY` and `VIEW_GROUP_STATISTICS`; every pre-existing role receives both grants to preserve the formerly implicit read behavior, and every future starter role receives them. New custom roles remain empty by default, and the optional minimal claim role receives only `CREATE_OWN_BOOKING`. No existing group is enabled and no identity, membership, role, or invitation is classified or converted automatically.

The API and compiled SPA ship in one image and must be upgraded together. `Membership.userId` remains required, while `Membership.email` and statement email become nullable only for opt-in guest data. Strict generated consumers must adopt the v0.6 contract before a group enables guests. Operators create and verify a backup before the forward-only migration, deploy the matching image, wait for readiness and foreign-key verification, then activate groups individually. Downgrade requires the prior image and pre-upgrade backup; toggling the feature off is not a schema rollback and deliberately preserves guest identities and history.
...
- Table-driven policy coverage for all twelve keys, role union, every implication edge, immediate revocation, group isolation, accepted group scopes, and rejected category/product scopes.
- Managed-guest migration coverage for credential coupling, required membership user IDs, nullable membership/statement email, disabled defaults, tenant-bound role and claim references, preservation of all pre-existing data, and backward-compatible read grants on every existing role.
- Guest service and HTTP coverage for atomic guest-settings transitions, exact-minimal role validation, default-role replacement, assigned-role switch conflicts, privacy-gated directory/statistics, minimal booking context, inline batch rollback/replay, notification-email suppression, rename/archive, conflict-protected claim creation, and history-preserving claim acceptance.
