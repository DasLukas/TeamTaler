...
`.codex/environments/environment.toml` exposes the **Start test server** action through `make test-server`. `scripts/test-server.sh` reads only an allowlisted SMTP variable set from the ignored `.env.test-server.local` file without evaluating it as shell code, builds the backend and fixture binaries, creates a permission-restricted temporary data directory below the ignored `tmp/test-server` path, seeds it through domain services, starts the backend on loopback port 8080, and starts Vite on loopback port 5173 with demo transport explicitly disabled. The fixture binary embeds local product and fictional profile images, normalizes them through `internal/media`, and attaches them through the same catalog and authentication services used by HTTP uploads; group settings receive deterministic booking and payment reason suggestions through `internal/groups`. Incomplete local SMTP credentials disable email delivery without blocking the server. Vite proxies `/api` to the real backend. Signal handling terminates both child processes and deletes only the action-owned temporary database, so every run begins from the same logical fixture without touching operator data in `data/`.
