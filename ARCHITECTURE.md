...
Catalog removal is deliberately two-stage. Categories and products are first archived through their existing versioned update commands; permanent `DELETE` commands require the archived state and a matching `If-Match` version. A product can be deleted only when no booking, including a voided booking, references it. A category can be deleted only after its products have been removed and when neither bookings nor ledger entries reference it. Deletion, audit recording, invitation-grant cleanup, and product-create idempotency cleanup are transactional. Immutable booking snapshots, ledger entries, and audit events are never deleted. Product image hashes that become unreferenced remain available for deliberate offline cleanup rather than being removed during a concurrent request.
