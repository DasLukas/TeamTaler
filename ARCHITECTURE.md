...
- `internal/groups` implements group creation, tenant membership lookup, administrator-managed identity and branding, member listing/archival, cumulative roles, category grants, permission replacement, individual invitation creation/editing/revocation/resending, shared invitation deduplication, and atomic idempotent CSV invitation imports.
...
- `internal/finance` implements consolidated member accounts, a group-wide active/former-membership balance summary, personal and anonymous group category statistics, incoming payments, payment reversal, recent ledger activity, and finance-manager read models.
...
- `web/src/features` contains authentication, dashboard, booking, activity, account, notifications, role-aware catalog, role-aware finance, and administration slices. The catalog slice owns the guarded `/catalog` workspace, contextual category/product creation, and the accessible icon picker. The finance slice owns the `/finance` overview, payment, and settlement tabs; the dashboard owns both personal overview information and anonymous group aggregates. The shared category-icon renderer maps persisted semantic icon names to tree-shaken Lucide components.
...
| `invitations` | Hashed single-use tokens, invited email, optional display name, group roles, category-grant defaults, expiry, and acceptance/revocation state. |
...
1. The administrator creates one invitation manually, including optional display-name and permission defaults, or uploads at most 256 KiB and 100 CSV data rows with an idempotency key. CSV invitations intentionally carry no extra roles or category grants.
...
8. The public preview endpoint returns only a suggested display name and an existing-account flag. Acceptance atomically consumes roles and category grants; a matching archived membership is reactivated under the same ID with fully replaced permissions. Existing accounts retain their global display name.
...
- Product creation, CSV invitation import, invitation resend, booking creation/reversal, payment creation/reversal, and period close require an `Idempotency-Key`.
