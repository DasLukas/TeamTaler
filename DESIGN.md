...
The statistics workspace is a simple overview for hobby and community groups rather than a business-intelligence report. Its hierarchy is compact range controls, one shared status line, two clearly distinct booking indicators, historical product and category visuals as purchase orientation, compact booking activity, then one central financial trend. Low-action-value membership composition, account-state distribution, booking-transaction count, category-charge, ledger-reconciliation, percentage-comparison, and secondary finance panels are intentionally absent. The workspace has no tabular view and no view switch: the group feature switch and one `VIEW_STATISTICS` grant expose the complete dashboard atomically. Panels use clear borders, tabular numerals, restrained radii, plain-language explanations, and a narrow semantic status rail. The active range, authoritative group time zone, generation time, and explicit refresh action remain visible so a chart is never detached from its measurement context. The URL owns only the range; a custom interval uses two clearly labelled calendar-date inputs and remains usable with keyboard, touch, and browser Back/Forward navigation.
