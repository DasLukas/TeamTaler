 ...
 * @returns A localized active, archived, or deleted status badge.
 */
export function MembershipStatusBadge({ status }: MembershipStatusBadgeProps) {
  const { t } = useTranslation();
  const label = status === 'ACTIVE' ? t('financeWorkspace.active') : status === 'ARCHIVED' ? t('common.archived') : t('common.deleted');
