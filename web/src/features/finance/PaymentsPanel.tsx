...
export function PaymentsPanel() {
  ...
  const paymentMutation = useMutation({
    mutationFn: () => api.createPayment(activeGroupId, { membershipId: membershipId || defaultAccount?.membershipId || '', amount: { minorUnits: parseMajorUnits(amount, activeGroup.currency), currency: activeGroup.currency }, receivedAt, method, reference: reference.trim() || undefined }),
    onSuccess: async () => {
      setDialogOpen(false);
      setAmount('');
      setReference('');
  ...
  if (paymentsQuery.isLoading || accountsQuery.isLoading) return <div className={styles.state}><StatePanel kind="loading" /></div>;
  if (!paymentsQuery.data || !accountsQuery.data) return <div className={styles.state}><StatePanel kind="error" message={t('finance.error')} /></div>;
