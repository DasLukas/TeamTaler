...
const roles: Role[] = [
  { id: 'role-admin', presetKey: 'GROUP_ADMINISTRATOR', name: 'Group administrator', grants: [{ permission: 'GROUP_ADMINISTRATION', scope: { type: 'GROUP' } }], version: 1, memberCount: 1, pendingInvitationCount: 0 },
...
describe('dynamic role controls', () => {
  ...
  it('allows assigning starter roles while protecting administrative roles', async () => {
    ...
    render(<RoleMultiSelect canManageRoles label="Roles" onChange={onChange} roleIds={[]} roles={roles} />);
  ...
  it('allows group-only administrators to transfer only the reserved administrator role', async () => {
    const user = userEvent.setup();
    const onChange = vi.fn();
